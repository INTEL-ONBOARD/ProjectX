...
const TaskCard: React.FC<TaskCardProps> = ({ task, index, onClick, onMoveTask, onDeleteTask, todayMode }) => {
  ...
  return (
    ...
    >
      ...
      <h3 className="font-semibold text-gray-900 text-[15px] mb-0.5 leading-snug">{task.title}</h3>
      {task.taskNumber != null && (
        <span className="text-[10px] font-mono text-gray-400 mb-1 block">#{String(task.taskNumber).padStart(3, '0')}</span>
      )}
      {task.description && (
